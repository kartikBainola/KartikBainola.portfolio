...
const normalizeItem = (item: DepthCarouselItem): NormalizedItem =>
  typeof item === "string"
    ? { image: item, alt: "" }
    : {
        image: item.image ?? "",
...
function CardMedia({ item, active }: { item: NormalizedItem; active: boolean }) {
...
export default function DepthCarousel({
  ...
  const startIndex = count ? clamp(initialIndex, 0, count - 1) : 0;
  ...
  const layout = useCallback((pos: number) => {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      const az = Math.abs(distance);
      const shown = az <= cfg.visibleCards + 0.5;

      const tz = -cfg.depth * distance;
      const tx = dir * cfg.spread * distance;
      const ry = dir * cfg.tilt * clamp(distance, 0, 1);
      ...
      const zi = Math.round(2000 - distance * 20);
  ...
  return (
    <div
      ...
      aria-label="Featured app previews"
      tabIndex={0}
    ...
    >
      <div className="depth-carousel__stage">
        {data.map((item, index) => {
          const cardClass = `depth-carousel__card${item.video ? " depth-carousel__card--device" : ""}`;
          const cardStyle = { width: cardWidth, height: cardHeight, borderRadius: radius };
          const content = (
            <>
              <CardMedia item={item} active={active === index} />
              <span
              ...
              {(item.title || item.caption) && (
                <div className="depth-carousel__caption">
                  {item.title ? <p className="depth-carousel__caption-title">{item.title.toUpperCase()}</p> : null}
                  {item.caption ? <span className="depth-carousel__caption-meta">{item.caption}</span> : null}
                </div>
              )}
            </>
          );
          ...
          if (item.href) {
            return (
              <Link
                ...
                data-cursor={active === index ? "project" : "button"}
                onClick={(event) => {
          ...
          return (
            <div
              key={`${item.alt || item.video || item.image}-${index}`}
              {...sharedProps}
              ref={(el) => {
                cardRefs.current[index] = el;
              }}
